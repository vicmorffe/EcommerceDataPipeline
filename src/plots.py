...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    return sns.scatterplot(data=df, x="product_weight_g", y="freight_value")
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    for holiday in df.values:
        if holiday[2]==True:
          holiday_date = datetime.datetime.strptime(str(holiday[1])[:10], "%Y-%m-%d")
          plt.axvline(holiday_date, color='blue', linestyle='--', linewidth=1)
