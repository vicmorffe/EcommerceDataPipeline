...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = pd.merge(pd.merge(orders, items, on=['order_id'], how='inner'), products, on='product_id', how='inner')
    ...
    deliveredMask=data['order_status']=='delivered'
    delivered = data[deliveredMask]
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    filtered_dates = orders[
      (orders['order_purchase_timestamp'] >= '2017-01-01 00:00:00') 
      & 
      (orders['order_purchase_timestamp'] < '2018-01-01 00:00:00')
      ]
    ...
    holidays['date'] = holidays['date'].apply(lambda x: str(x)[:10])
    
    order_purchase_ammount_per_date = filtered_dates.groupby('filtered_dates_day').count().reset_index()
    ...
    result_df = order_purchase_ammount_per_date[['filtered_dates_day','order_id']]    
    result_df=result_df.rename(columns={'filtered_dates_day': 'date'})
    result_df = pd.merge(result_df, holidays, on=['date'], how='left')
    result_df = result_df[result_df.columns[:3]]
    result_df=result_df.rename(columns={result_df.columns[2]: 'holiday'})
    result_df=result_df.fillna(False)
    result_df['holiday'][result_df['holiday']!=False] = True
    

    result_df = result_df.rename(columns={'order_id': 'order_count'})
    result_df = result_df[['order_count','date','holiday']]
    result_df['date'] = pd.to_datetime(result_df['date'])

    #dfResult['date'] = dfResult['date'].apply(lambda x: x.timestamp())
